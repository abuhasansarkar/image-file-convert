...
export function ConversionPageTemplate({
  ...
  const { files, addFiles, clearFiles, isProcessing, setProcessing } =
    useConversionStore();
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      toast.success(
        "Download started",
        `Downloading ${filesToZip.length} converted files as ZIP.`
      );
    } catch (error) {
      console.error("Download error:", error);
      toast.error(
        "Download failed",
        "Could not create ZIP file. Please try downloading files individually."
      );
    }
  };
